...
export class ProBookDialogComponent implements OnInit {
  username: any;
  constructor(private formBuilder: FormBuilder, private apptserv: AppointmentService, @Inject(MAT_DIALOG_DATA) public meetingData: Appointment, private dialogRef: MatDialogRef<ProBookDialogComponent>,private _snack:MatSnackBar) { }
    ...
    this.apptserv.checkPatientAppointmentClash(this.pat.patient_id, this.apptDate, fr_time).subscribe(resp => {
      ...
      if (!clash) {
        ...
        this.apptserv.createAppt(this.newAppt).subscribe(resp => {
          ...
          this._snack.open("Appointment Created with Meeting ID: " + createdAppt.meetId)
          duration: 3000
          }
  ...
  updateAppt() {
    ...
    this.apptserv.checkPatientAppointmentClash(this.pat.patient_id, this.apptDate, fr_time).subscribe(resp => {
      ...
      if (!clash) {
        ...
        console.log("Meeting Data Model: "+ Object.values(this.newAppt) )
  ...
  populateMeetingDataModel() {
    ...
    console.log("Descipriotn test: "+this.newAppt.description);
  }
