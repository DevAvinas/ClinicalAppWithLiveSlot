...
export class AppointmentService {
  ...
  getSlotsOfPhy(phyId:string,apptDate:string):Observable<any> {
    ...
    return this.http.get<any>("http://127.0.0.1:8084/api/private/appt/slots/"+phyId+"/"+apptDate,{'headers': new HttpHeaders().set('Authorization',`Bearer ${token}`)});
   }
  ...
  getbypatientid(patid:any):Observable<PatientMaster>{
    let token=this.authService.getToken();
    return this.http.get<PatientMaster>("http://127.0.0.1:8082/api/private/getbypatientid/"+patid,{'headers': new HttpHeaders().set('Authorization',`Bearer ${token}`)});
  }
