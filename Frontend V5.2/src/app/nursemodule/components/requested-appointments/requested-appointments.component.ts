...
export class RequestedAppointmentsComponent implements OnInit {
 ...
 constructor(private _snack:MatSnackBar,private patientvisitService:PatientVisitDetailsService,private route: ActivatedRoute,private dialog:MatDialog) { 
 ...
 ngOnInit(): void {
   ...
   this.getRequestedAppointmentsByStatus();
   this.temporary=this.route.snapshot.paramMap.get('meetId')
 }
 getRequestedAppointmentsByStatus(){
  console.log("status------------->"+status)
  this.patientvisitService.getRequestedAppointmentsByStatus().subscribe(obj=>{
...
rescheduleFunction(meetingdata:any){
  this.dialog.open(ProBookDialogComponent, { width: '100%',data:meetingdata}).afterClosed().subscribe(resp =>this.getRequestedAppointmentsByStatus());
}
