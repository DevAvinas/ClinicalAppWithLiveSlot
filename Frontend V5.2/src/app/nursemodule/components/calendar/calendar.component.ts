...
export class CalendarComponent implements OnInit {
  ...
  constructor(private _snack:MatSnackBar, private patientvisitService:PatientVisitDetailsService,private dialog: MatDialog,private medservice:MedicationDetailsService) { }
  ...
  ngOnInit(): void {
   
    {
      ...
      this.patientvisitService.getUpcomingAppointmentsByStatus().subscribe(data => {
  ...
  onPopupOpen(args: PopupOpenEventArgs): void {
    ...
    if (args.type === 'Editor' || !subExists) {
      ...
      if(strttime.getTime()<=todayDt.getTime())  {
        this._snack.open("Plese Select an Upcoming Appointment ","Ok",{
          duration: 3000
        });
      }
